...
# Create state table to track processed Silver files (incremental processing)
con.execute("""
    CREATE TABLE IF NOT EXISTS processed_silver_files (
...
processed_files_result = con.execute("SELECT file_path FROM processed_silver_files").fetchall()
processed_files_set = {row[0] for row in processed_files_result}
print(f"📋 Found {len(processed_files_set)} already processed Silver files")
...
silver_files = [f for f in all_silver_files if f not in processed_files_set]

if not silver_files:
    print(f"✅ All {len(all_silver_files)} Silver files already processed. Nothing new to process.")
    con.close()
    sys.exit(0)

print(f"📦 Found {len(all_silver_files)} total Silver files, {len(silver_files)} new files to process")
...
for file in silver_files:
    ...
    # Fast ML classification for categories (supports multi-category, many-to-many)
    try:
        if {"description", "merchant"}.issubset(cols):
            ...
            # Fast batch multi-category classification
            cat_lists = classifier.classify_batch_multi(
                df_cat["description"].fillna("").astype(str).tolist(),
                df_cat["merchant"].fillna("").astype(str).tolist()
            )
            
            df_cat["categories"] = cat_lists
            # Explode to one row per (description, merchant, category)
            df_cat = df_cat.explode("categories").rename(columns={"categories": "category"})
            df_cat = df_cat.dropna(subset=["category"])
            df_cat["category_id"] = df_cat.apply(
                lambda r: hash_key(str(r["category"]) + str(r["merchant"])), axis=1
            )
    ...
    # Process fact table (critical step - file only marked as processed if this succeeds)
    fact_success = False
    try:
        if required_cols_fact.intersection(cols):
            ...
            # Fast ML classification for fact table (many-to-many)
            if {"description", "merchant"}.issubset(df.columns):
                ...
                df["categories"] = cat_lists
                # For compatibility, keep a "primary" category in the fact table:
                ...
                df["category_id"] = df.apply(
                    lambda r: hash_key(str(r["primary_category"]) + str(r["merchant"]))
                    if r.get("primary_category") is not None
                    else None,
                    axis=1,
                )
            else:
            ...
            # FACT table no longer stores category_id (handled via bridge table)
            df_fact = df[["transaction_id", "date_id", "user_id", "payment_id", "amount"]].copy()
            df_fact.columns = ["transaction_id", "date_id", "user_id", "payment_id", "transaction_amount"]

            con.register("tmp_fact", df_fact)
            con.execute("""
                INSERT OR IGNORE INTO transaction_fact
            (transaction_id, date_id, user_id, payment_id, transaction_amount)
            SELECT transaction_id, date_id, user_id, payment_id, transaction_amount
                FROM tmp_fact;
            """)
            print(f"✔ FACT rows inserted ({len(df_fact)} rows)")

            # Build many-to-many bridge rows for all categories per transaction
            if "categories" in df.columns and "merchant" in df.columns:
                bridge_rows = []
                for _, row in df[["transaction_id", "merchant", "categories"]].iterrows():
                    ...
                    for cat in cats:
                        if cat is None:
                            continue
                        cat_id = hash_key(str(cat) + str(row["merchant"]))
                        bridge_rows.append(
                            {"transaction_id": row["transaction_id"], "category_id": cat_id}
                        )

                if bridge_rows:
                    df_bridge = pd.DataFrame(bridge_rows).drop_duplicates()
                    con.register("tmp_bridge", df_bridge)
                    con.execute("""
                        INSERT OR IGNORE INTO transaction_category_bridge
                        (transaction_id, category_id)
                        SELECT transaction_id, category_id
                        FROM tmp_bridge;
                    """)
                    print(f"✔ BRIDGE rows inserted ({len(df_bridge)} rows)")
    ...
    # Mark this Silver file as processed ONLY if fact table insertion succeeded
    if fact_success:
        try:
            con.execute(f"""
                INSERT OR IGNORE INTO processed_silver_files (file_path)
                VALUES ('{file}')
            """)
            print(f"✅ Silver file marked as processed: {file}")
        except Exception as e:
            print(f"⚠️ Error marking file as processed: {e}")
    else:
        print(f"⚠️ Silver file NOT marked as processed (fact insertion failed) - will retry on next run")
...
tables = {
    ...
    "transaction_category_bridge": f"{gold_prefix}/transaction_category_bridge.parquet",
}
